...
const Tab = styled(motion.button)<{ active: boolean }>(props => ({
	...
	color: props.active ? colorToken.gray600 : colorToken.gray300,
	fontFamily: 'SUITRegular',
...
const AccountButton = styled(motion.button)<{ isOpen?: boolean }>(({ isOpen }) => ({
	width: '100%',
	padding: '10px 10px',
	display: 'flex',
	flexDirection: 'column',
...
const ContactPage = () => {
	...
	const toggleAccountModal = (name: string) => {
		setShowAccountModals(prev => (prev.includes(name) ? prev.filter(n => n !== name) : [...prev, name]));
	};
	...
	return (
		<Container>
			...
			<ContactList initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
				<AnimatePresence mode="wait">
					...
					>
						{(activeTab === 'groom' ? groomContacts : brideContacts).map((contact, index) => (
							<ContactCard
								key={contact.role}
								...
								<div>
									...
									>
										...
										>
											...
											<motion.span
												animate={{
													rotate: showAccountModals.includes(contact.name) ? -180 : 0,
												}}
												transition={{ duration: 0.3 }}
											>
												<DownSvg
